...
function [getCodes] = MHOQ_FMIN(Xcs, N, Nb, getControl, x_init, Qmodel, YQns, MLns, A, B)
    f = waitbar(0, 'Starting');
    C_MHOQ = [];
    len_MPC = length(Xcs)-N;
    bv_kminus1 = zeros(2^Nb,1);

    for k = 1:len_MPC
        ref_in = Xcs(k:k+N-1) ;                          % intance t = k to t = k+N
        inputs = {x_init, ref_in, bv_kminus1};       % Inputs: Initial state and reference value at t = k
