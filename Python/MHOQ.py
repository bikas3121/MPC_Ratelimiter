...
class MHOQ:
    ...
    def get_codes(self, N_PRED, Xcs, YQns, MLns ):
        ...
        # Initial state
        init_state = np.ones(x_dim).reshape(-1,1)
        err_out = np.zeros_like(Xcs)
        # MPC loop
        for j in tqdm.tqdm(range(len_MPC)):
            ...
            X_OPT = values[N_PRED+x_dim:N_PRED + 2*x_dim]
            ...
            # x0_new = self.state_prediction(X_OPT.reshape(-1,1), con)
            
            err_out[j] = self.C@x0_new + con
        ...
        return np.array(CD).reshape(1,-1), err_out 
